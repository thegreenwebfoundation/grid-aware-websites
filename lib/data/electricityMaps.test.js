...
test("getGridIntensity requires a zone and API key", async () => {
    ...
    await expect(electricityMaps("DE", import.meta.env.VITE_EMAPS_API_KEY)).resolves.toBeDefined();
});

// Write a test to check for a valid zone
test("getGridIntensity checks for a valid zone", async () => {
    await expect(electricityMaps("ZZ", import.meta.env.VITE_EMAPS_API_KEY)).resolves.toEqual({
        status: "error",
