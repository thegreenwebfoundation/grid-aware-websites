...
test("fetchGridIntensity throws an error if the mode is invalid", async () => {
  ...
  expect(() => gridIntensity.setOptions(options)).toThrow(
    "Invalid mode. Mode must be 'average' or 'limit'.",
  );
});
...
// Test to check that level mode is used by default
test("fetchGridIntensity returns the correct default mode", async () => {
  const zone = "DE";
  const result = await gridIntensity.check(zone);
  expect(result.data).toHaveProperty("mode", "level");
});

// Test to check the response structure when mode is 'level'
test("fetchGridIntensity returns correct keys for level mode", async () => {
  const zone = "DE";
  const options = { mode: "level" };
...
// Test that using "average" with a {lat, lon} is not supported
test("fetchGridIntensity returns an error when using 'average' mode with lat/lon coordinates", async () => {
  const coords = { lat: "51.1", lon: "0.1" };
  const options = { mode: "average" };
  gridIntensity.setOptions(options);
  const result = await gridIntensity.check(coords);
  expect(result).toHaveProperty("status", "success");
  expect(result).toHaveProperty("region", "GB");
  expect(result.data).toHaveProperty("averageIntensity");
});
...
test("fetchGridIntensity throws an error if the zone parameter is invalid", async () => {
  ...
  await expect(gridIntensity.check(zone2)).resolves.toHaveProperty(
    "status",
    "success",
  );

  const zone3 = { lat: "51.1", lon: "0.1" }; // Valid object
  await expect(gridIntensity.check(zone3)).resolves.toHaveProperty(
    "status",
    "success",
  );

  const zone4 = { region: "DE" }; // Valid zone type, but eventually it should fail because it's not a valid zone
  await expect(gridIntensity.check(zone4)).resolves.toHaveProperty(
    "status",
    "error",
  );
});
...
test("fetchGridIntensity throws an error if the apiKey parameter is missing", async () => {
  const gridIntensity2 = new GridIntensity();
  const zone = "DE";
