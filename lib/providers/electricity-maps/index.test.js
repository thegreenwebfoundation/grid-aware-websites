...
test("getGridIntensity requires a zone and API key", async () => {
  ...
  await expect(
    getGridIntensity("DE", "level", import.meta.env.VITE_EMAPS_API_KEY),
...
test("getGridIntensity checks for a valid zone", async () => {
  ...
  const result = await getGridIntensity(zone, "level", apiKey);
...
// Test successful API response for average
test("getGridIntensity returns correct data structure on success for average API", async () => {
  const result = await getGridIntensity(
    "DE",
    "average",
    import.meta.env.VITE_EMAPS_API_KEY,
  );
...
// Test successful API response for level
test("getGridIntensity returns correct data structure on success for level API", async () => {
  const result = await getGridIntensity(
    "DE",
    "level",
    import.meta.env.VITE_EMAPS_API_KEY,
  ...
  expect(result).toHaveProperty("status", "success");
  expect(result).toHaveProperty("data");
  expect(result.data).toHaveProperty("zone", "DE");
  expect(result.data.data[0]).toHaveProperty("level");
  expect(result.data.data[0]).toHaveProperty("datetime");
});
...
test("getPowerBreakdown returns correct data structure on success", async () => {
  const result = await getPowerBreakdown(
    "DE",
    import.meta.env.VITE_EMAPS_API_KEY,
  );
  expect(result).toHaveProperty("status", "success");
