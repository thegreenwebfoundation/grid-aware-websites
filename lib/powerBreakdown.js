import { getPowerBreakdown as electricityMapsPowerBreakdown } from "./data/electricityMaps.js";
...
class PowerBreakdown {
  ...
  setMode(mode) {
    if (mode !== "renewable" && mode !== "low-carbon") {
      throw new Error("Invalid mode. Mode must be 'renewable' or 'low-carbon'.");
    }
    this.mode = mode;
  }
  ...
  setMinimumPercentage(minimumPercentage) {
    if (isNaN(minimumPercentage) || minimumPercentage < 0 || minimumPercentage > 100) {
      throw new Error(
