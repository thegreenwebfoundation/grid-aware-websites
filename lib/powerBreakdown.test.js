...
test("fetchPowerBreakdown throws an error if the zone parameter is invalid", async () => {
  ...
  await expect(powerBreakdown.check(zone2)).resolves.toHaveProperty("status", "success");

  const zone3 = { lat: 51.1, lon: 0.1 }; // Valid object
  await expect(powerBreakdown.check(zone3)).resolves.toHaveProperty("status", "success");

  const zone4 = { region: "DE" }; // Valid zone type, but eventually it should fail because it's not a valid zone
  await expect(powerBreakdown.check(zone4)).resolves.toHaveProperty("status", "error");
});
